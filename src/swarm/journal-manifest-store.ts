...
/**
 * The read-only journal manifest scan — the exchange root bounds the LIVE set
 * (the journal outlives pruned task dirs); per surviving dir the truth is the
 * journal replay; foreign-backend entries are filtered like every scan.
 * Extracted so the #30 read API (`swarm snapshot`, src/swarm/snapshot.ts) can
 * serve the journal-mode manifest projection with ZERO writes — the store
 * constructor opens a WRITER (create/migrate on open), which a pure read must
 * never do. One spelling of the scan (Law 9); both callers share it.
 * <p>
 * FUNCTION_CONTRACT:
 * Input:
 *   - reader — a journal reader (eventsForTask only)
 *   - sessionIdFor — the fleet session-id spelling for a task dir
 *   - backendName — the caller's active backend (foreign-backend filter)
 * Output: every replayable manifest of task dirs existing under exchangeRoot()
 * Guarantees: pure read (no writer, no projection write); tolerant (missing
 *   root → [], unreadable dirs skipped); never throws
 * Raises: never
 */
export function scanManifestsViaJournalReader(
	reader: Pick<JournalReader, "eventsForTask">,
	sessionIdFor: (dir: string) => string,
	backendName: string,
): ExchangeManifest[] {
	...
	for (const taskName of taskNames) {
		const dir = resolve(root, taskName);
		const m = replayManifest(dir, reader.eventsForTask(sessionIdFor(dir), taskName));
...
export function createJournalManifestStore(opts: JournalManifestStoreOptions = {}): JournalManifestStore {
	...
	const store: JournalManifestStore = {
		...
		scan(backendName) {
			// The ONE read-only journal scan (shared with the #30 read API — Law 9:
			// one implementation per mechanism).
			return scanManifestsViaJournalReader(reader, sessionIdFor, backendName);
		},
		close() {
			writer.close();
			reader.close();
