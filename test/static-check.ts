...
const decompositionLedger: ReadonlyArray<{ file: string; owner: string; targetRelease: string; plan: string }> = [
	...
	{ file: "src/watch-config.ts", owner: "operator", targetRelease: "1.19.0", plan: "extract the collect/swarm/schedule section resolvers into sibling config modules (the watch resolver stays); #10's scheduled-wake limits pushed the shared tolerant-reader module over the threshold" },
